...
local function readMeter(name)
    ...
    if not meter or type(meter.getStress) ~= "function" or type(meter.getStressCapacity) ~= "function" then
        return nil, nil
    end
...
local function steamKey(reactor)
    return reactor.id .. "_steam"
end

local function backupKey(reactor)
    return reactor.id .. "_backup"
end
...
local function buildSeries()
    ...
    for _, reactor in ipairs(cfg.reactors or {}) do
        table.insert(series, {
            key = steamKey(reactor),
            label = reactor.label .. " Steam",
        ...
        table.insert(series, {
            key = backupKey(reactor),
            label = reactor.label .. " Backup",
            color = graph.seriesColor(index),
            symbol = graph.seriesSymbol(index)
        })
        index = index + 1
...
local function collectSample()
    ...
    for _, reactor in ipairs(cfg.reactors or {}) do
        local steamUsed, steamCapacity = readMeter(reactor.steamStressometer)
        local backupUsed, backupCapacity = readMeter(reactor.backupStressometer)

        values[steamKey(reactor)] = steamUsed
        values[backupKey(reactor)] = backupUsed

        capacityMax = math.max(capacityMax, steamCapacity or 0, backupCapacity or 0)
    end
...
local function render(target)
    graph.renderGraph(target, {
        ...
        subtitle = "Steam and backup network used SU | sample " .. sampleSeconds .. "s | points " .. historyPoints,
        headerColor = colors.blue,
        series = buildSeries(),
        history = history,
        yMin = 0,
        yMax = lastCapacityMax,
        valueSuffix = " SU"
    })
end
