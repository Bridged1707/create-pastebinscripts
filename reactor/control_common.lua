...
function M.getRelayState(relayConfig)
    ...
    local relay = peripheral.wrap(relayConfig.peripheral)
    if not relay then return nil, "MISSING" end
...
function M.setRelayState(relayConfig, enabled)
    ...
    local relay = peripheral.wrap(relayConfig.peripheral)
    if not relay then return false, "MISSING" end
...
function M.readSpeedometer(name)
    if isPlaceholder(name) then return { online = false, rpm = 0, status = "UNCONFIGURED" } end
    local meter = peripheral.wrap(name)
    if not meter then return { online = false, rpm = 0, status = "MISSING" } end
    if type(meter.getSpeed) ~= "function" then return { online = false, rpm = 0, status = "BAD TYPE" } end
    local ok, speed = pcall(meter.getSpeed)
    if not ok then return { online = false, rpm = 0, status = "READ ERR" } end
    return { online = true, rpm = tonumber(speed) or 0, status = "OK" }
end
